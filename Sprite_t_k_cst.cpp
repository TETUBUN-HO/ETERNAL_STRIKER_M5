...
*************************************************************************************x*/
void TFT_eSprite_CST::setScrollRect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint16_t color)
{
...
*************************************************************************************x*/
void TFT_eSprite_CST::scroll(int16_t dx, int16_t dy)
{
...
*************************************************************************************x*/
void TFT_eSprite_CST::fillSprite(uint32_t color)
{
  if (!_created ) return;
...
//void TFT_eSprite_CST::setCursor(int16_t x, int16_t y)
//{
//  this->cursor_x = x;
//  this->cursor_y = y;
//}
...
int16_t TFT_eSprite_CST::width(void)
{
...
*************************************************************************************x*/
int16_t TFT_eSprite_CST::height(void)
{
  if (!_created ) return 0;
...
void TFT_eSprite_CST::setRotation(uint8_t rotation)
{
  _rotation = rotation;
...
uint8_t TFT_eSprite_CST::getRotation(void)
{
  return _rotation;
}
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawPixel(uint32_t x, uint32_t y, uint32_t color)
{
...
/***************************************************************************************
** Function name:           drawPixel_len //2018/10/3 T.K add
** Description:             push a multi pixel at an arbitrary position
*************************************************************************************x*/
void TFT_eSprite_CST::drawPixel_len(uint32_t x, uint32_t y, uint32_t * color_t, uint32_t len)
{
  // x and y are unsigned so that -ve coordinates turn into large positive ones
  // this make bounds checking a bit faster
	int i;
  if ((x >= _iwidth) || (y >= _iheight) || !_created) return;
	...
	if (_bpp == 16)
	{
		yb=x+y*_iwidth;
		for(i=0;i<len;i++){
			if ((x+i >= _iwidth)) break;
			color = color_t[i];
			color = (color >> 8) | (color << 8);
			_img[yb] = (uint16_t) color;
			yb++;
		}
	}
	else if (_bpp == 8)
	{	yb=x+y*_iwidth;
		for(i=0;i<len;i++){
			if ((x+i >= _iwidth)) break;
			color = color_t[i];
			_img8[yb] = (uint8_t)((color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3);
			yb++;
		}
	}
	else // 1 bpp
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color)
{
  if (!_created ) return;
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawFastVLine(int32_t x, int32_t y, int32_t h, uint32_t color)
{
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawFastHLine(int32_t x, int32_t y, int32_t w, uint32_t color)
{
...
*************************************************************************************x*/
void TFT_eSprite_CST::fillRect(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color)
{
  if (!_created ) return;

  if (x < 0) { w += x; x = 0; }
  if (y < 0) { h += y; y = 0; }
  if ((x + w < 0) || (y + h < 0) || (x >= _iwidth) || (y >= _iheight)) return;
  if ((x + w) > _iwidth)  w = _iwidth  - x;
  if ((y + h) > _iheight) h = _iheight - y;
  if ((w < 1) || (h < 1)) return;
...
*************************************************************************************x*/
size_t TFT_eSprite_CST::write(uint8_t utf8)
{
  if (utf8 == '\r') return 1;
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawChar(int32_t x, int32_t y, unsigned char c, uint32_t color, uint32_t bg, uint8_t size)
{
  if (!_created ) return;
...
*************************************************************************************x*/
int16_t TFT_eSprite_CST::drawChar(unsigned int uniCode, int x, int y)
{
  return drawChar(uniCode, x, y, textfont);
}

int16_t TFT_eSprite_CST::drawChar(unsigned int uniCode, int x, int y, int font)
{
  if (!_created ) return 0;
...
*************************************************************************************x*/
void TFT_eSprite_CST::drawGlyph(uint16_t code)
{
  if (code < 0x21)
  {
    if (code == 0x20) {
...
*************************************************************************************x*/
void TFT_eSprite_CST::printToSprite(String string)
{
...
** Description:             Write a string to the sprite cursor position
*************************************************************************************x*/
void TFT_eSprite_CST::printToSprite(char *cbuffer, int len) //String string)
{
  if(!this->fontLoaded) return;
...
*************************************************************************************x*/
int16_t TFT_eSprite_CST::printToSprite(int16_t x, int16_t y, uint16_t index)
{
  bool newSprite = !_created;
